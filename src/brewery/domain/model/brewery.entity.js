...
export class Brewery {
  ...
  /**
   * Obtiene la URL de la imagen basada en el nombre de la cervecería usando Clearbit Logo API
   * @returns {string} URL de la imagen del logo
   */
  getLogoUrl() {
  ...
  /**
   * Extrae el dominio de la URL del sitio web
   * @returns {string} Dominio del sitio web
   */
  getDomainFromUrl() {
  ...
  /**
   * Convierte el nombre de la cervecería a formato kebab-case para URLs
   * @returns {string} Nombre en formato kebab-case
   */
  getKebabCaseName() {
    if (!this.name) return '';
    return this.name
